...
def run_quasi_newton_updates(
    ...
    ------
    ValueError
        If ``n_iterations`` is negative or if the inverse Jacobian shape does
        not match the Jacobian size implied by ``dynamic_network_data``.
    """
    if n_iterations < 0:
    ...
    expected_size = pvpq_indices.size + pq_indices.size

    if jacobian_inv.shape != (expected_size, expected_size):
        raise ValueError("Inverse Jacobian shape does not match the Jacobian size implied by the dynamic network data.")

    if y_matrix is None:
        y_matrix = _get_admittance_matrix_from_network_data(dynamic_network_data)

    base_inverse_jacobian = np.asarray(jacobian_inv, dtype=float)
    ...
    for iteration_index in range(n_iterations):
        ...
        if denominator > regularization:
            ...
            a_factors[:, iteration_index] = new_a_factor
            b_factors[:, iteration_index] = new_b_factor
            n_active_factors += 1
