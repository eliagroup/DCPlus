...
def _collect_targeted_indices(
    ...
    reassigned_branch_indices = {int(branch_idx) for branch_idx in branches_connected_to_bus_b}
    _add_bus_component_indices(targeted_indices, bus_to_split, angle_idx_map, magnitude_idx_map)
    if include_new_bus:
        _add_bus_component_indices(
            targeted_indices,
            new_bus_b_index,
            angle_idx_map,
            magnitude_idx_map,
    ...
    for branch_idx in branch_indices_with_delta:
...
def _get_branch_row_component_indices(
    bus_from: int,
    bus_to: int,
    angle_idx_map: Int[np.ndarray, " n_eq_jacobian"],
    magnitude_idx_map: Int[np.ndarray, " n_eq_jacobian"],
    ...
    Int[np.ndarray, "4"]
        ...
        magnitude-to.
    """
    mag_idx_from = magnitude_idx_map[bus_from]
    mag_idx_to = magnitude_idx_map[bus_to]
    if reactive_power_bus_mask is not None:
        if not reactive_power_bus_mask[bus_from]:
            mag_idx_from = -1
        if not reactive_power_bus_mask[bus_to]:
            mag_idx_to = -1
...
def _get_branch_column_component_indices(
    bus_from: int,
    bus_to: int,
    angle_idx_map: Int[np.ndarray, " n_eq_jacobian"],
    magnitude_idx_map: Int[np.ndarray, " n_eq_jacobian"],
) -> Int[np.ndarray, "4"]:
    """Return the Jacobian state-column indices touched by one branch contribution."""
    return np.array(
        [
            angle_idx_map[bus_from],
            angle_idx_map[bus_to],
...
def _get_shunt_row_component_indices(
    bus_idx: int,
    angle_idx_map: Int[np.ndarray, " n_eq_jacobian"],
    magnitude_idx_map: Int[np.ndarray, " n_eq_jacobian"],
    reactive_power_bus_mask: np.ndarray | None = None,
    ...
    """
    mag_idx = magnitude_idx_map[bus_idx]
    if reactive_power_bus_mask is not None and not reactive_power_bus_mask[bus_idx]:
        mag_idx = -1
    return np.array([angle_idx_map[bus_idx], mag_idx], dtype=int)


def _get_shunt_column_component_indices(
    bus_idx: int,
    angle_idx_map: Int[np.ndarray, " n_eq_jacobian"],
    magnitude_idx_map: Int[np.ndarray, " n_eq_jacobian"],
) -> Int[np.ndarray, "2"]:
    """Return the Jacobian state-column indices touched by one shunt contribution."""
...
def _accumulate_sub_delta(
    ...
    row_component_indices: Int[np.ndarray, " m"],
    col_component_indices: Int[np.ndarray, " m"],
    ...
    row_component_indices : Int[np.ndarray, " m"]
        Jacobian row indices touched by the local contribution.
    col_component_indices : Int[np.ndarray, " m"]
        Jacobian column indices touched by the local contribution.
    position_lookup : dict[int, int]
    ...
    """
    valid_row_positions = np.flatnonzero(row_component_indices >= 0)
    valid_col_positions = np.flatnonzero(col_component_indices >= 0)
    if valid_row_positions.size == 0 or valid_col_positions.size == 0:
        return

    local_row_indices = row_component_indices[valid_row_positions]
    local_col_indices = col_component_indices[valid_col_positions]
    mapped_row_positions = [position_lookup[int(idx)] for idx in local_row_indices]
    mapped_col_positions = [position_lookup[int(idx)] for idx in local_col_indices]
    sub_delta = np.real(delta_matrix[np.ix_(valid_row_positions, valid_col_positions)])

    for row_offset, pos_row in enumerate(mapped_row_positions):
        for col_offset, pos_col in enumerate(mapped_col_positions):
...
def _accumulate_branch_reassignment_delta(
    ...
    reactive_power_bus_mask : np.ndarray | None, optional
        Optional mask indicating which buses retain reactive-power equation rows
        while assembling branch contribution rows.
    """
    reassigned_branch_indices = {int(branch_idx) for branch_idx in branches_connected_to_bus_b}
    ...
    for branch_idx in branch_indices_with_delta:
        ...
        _accumulate_sub_delta(
            ...
            row_component_indices=_get_branch_row_component_indices(
                from_bus_old,
                to_bus_old,
                angle_idx_map,
                magnitude_idx_map,
                reactive_power_bus_mask,
            ),
            col_component_indices=_get_branch_column_component_indices(
                from_bus_old,
                to_bus_old,
                angle_idx_map,
                magnitude_idx_map,
...
def _accumulate_shunt_reassignment_delta(
    ...
    reactive_power_bus_mask : np.ndarray | None, optional
        Optional mask indicating which buses retain reactive-power equation rows
        while assembling shunt contribution rows.
    """
    for shunt_idx in shunt_connected_to_bus_b:
        shunt_bus_old = int(shunt_to_bus[shunt_idx])
        shunt_bus_new = new_bus_b_index if shunt_bus_old == bus_to_split else shunt_bus_old
        ...
        _accumulate_sub_delta(
            delta_block=delta_block,
            delta_matrix=delta_old,
            row_component_indices=_get_shunt_row_component_indices(
                shunt_bus_old,
                angle_idx_map,
                magnitude_idx_map,
                reactive_power_bus_mask,
            ),
            col_component_indices=_get_shunt_column_component_indices(
                shunt_bus_old,
                angle_idx_map,
                magnitude_idx_map,
            ),
...
def _build_delta_block(
    ...
    angle_idx_map: Int[np.ndarray, " n_eq_jacobian"],
    magnitude_idx_map: Int[np.ndarray, " n_eq_jacobian"],
    reactive_power_bus_mask: np.ndarray | None,
    ...
    reactive_power_bus_mask : np.ndarray | None
        Optional mask indicating which buses retain reactive-power equation rows
        in the assembled local update block.
    apply_split_bus_adjustment : bool
...
def compute_bsdf_update(
    ...
    y_ff_base_arr = y_ff if y_ff_base is None else y_ff_base
    y_ft_base_arr = y_ft if y_ft_base is None else y_ft_base
    y_tf_base_arr = y_tf if y_tf_base is None else y_tf_base
    y_tt_base_arr = y_tt if y_tt_base is None else y_tt_base
