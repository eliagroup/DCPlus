...
ENABLED_LIMIT_BSDF_CASES = [
    bsdf_test_case
    for bsdf_test_case in get_bsdf_cases()
    if not bsdf_test_case.shunt_connected_to_bus_b_string and not bsdf_test_case.injections_connected_to_bus_b_string
]
...
def test_bsdf_full_rank_applies_phase_tap_change_before_split(
    ...
    jacobian_data_base = get_jacobian_data_from_network_data(dynamic_info_base)
    ...
    jacobian_data_tap = get_jacobian_data_from_network_data(dynamic_info_tap)
    ...
    branch_from_split = dynamic_info_tap.branch_from_bus.copy()
    branch_to_split = dynamic_info_tap.branch_to_bus.copy()
    ...
    dynamic_info_split_expected = replace_network_data(
    ...
    jacobian_data_split_expected = get_jacobian_data_from_network_data(dynamic_info_split_expected)
    ...
    jacobian_inv_bsdf = compute_bsdf_update(
        ...
        v_mag_hat=dynamic_info_split_expected.bus_voltage_magnitudes,
        theta_hat=dynamic_info_split_expected.bus_voltage_angles_rad,
