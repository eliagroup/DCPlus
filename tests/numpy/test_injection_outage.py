...
from dc_plus.interfaces.jacobian_network_data import _apply_jacobian_dx_to_network_data
...
def test_non_voltage_regulating_injection_outage_numpy(get_net) -> None:
    ...
    for injection_idx in _connected_injection_indices(injections):
        ...
        network_info_n1 = create_network_data_pypowsybl(net_n1)
        dynamic_info_n1 = network_info_n1.dynamic_network_data
        string_info_n1 = network_info_n1.string_network_data
...
@pytest.mark.parametrize("get_net", powsybl_networks)
def test_non_voltage_regulating_injection_outage_numpy_enabled_limit_path(get_net) -> None:
    ...
    outage_q = injections["setpoint_q"].fillna(injections["q"]).to_numpy(dtype=float)
    outage_rows = [
        np.array([injection_idx], dtype=np.int64)
        for injection_idx in _connected_injection_indices(injections)
        if (injection_powsybl_type.loc[injections.loc[injection_idx, "id_str"], "type"])
        not in (["HVDC_CONVERTER_STATION", "DANGLING_LINE"])
    ]
    if not outage_rows:
        pytest.skip(f"No supported injection outages available for {get_net.__name__}")

    outage_batch = _pad_outage_rows(outage_rows[: min(3, len(outage_rows))])
    dx_batch = non_voltage_regulating_injection_outage_dx(
        jacobian_inv=jacobian_data.inverse_jacobian,
        outage_injection_indices=outage_batch,
        injection_to_bus=dynamic_info.injection_to_bus,
    ...
    for case_idx, outage_row in enumerate(outage_batch):
        expected_delta_p, expected_delta_q = _expected_bus_delta(dynamic_info, outage_row, outage_p, outage_q)
        expected_mismatch = np.r_[
            expected_delta_p[jacobian_data.bus_angle_indices],
            expected_delta_q[jacobian_data.bus_magnitude_indices],
        ]
        expected_dx = -(jacobian_data.inverse_jacobian @ expected_mismatch)
        np.testing.assert_allclose(dx_batch[case_idx], expected_dx, rtol=TOL, atol=TOL)
        ...
        if np.any(expected_delta_q[jacobian_data.bus_magnitude_indices] != 0.0):
            assert not np.array_equal(
                updated_dynamic_info.bus_voltage_magnitudes[jacobian_data.bus_magnitude_indices],
                dynamic_info.bus_voltage_magnitudes[jacobian_data.bus_magnitude_indices],
            )
