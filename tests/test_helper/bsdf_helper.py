...
from dc_plus.numpy.injection_helpers import _aggregate_bus_injections
...
def _reassign_selected_bus_indices(
    ...
    for asset_idx in np.asarray(selected_asset_indices, dtype=np.int32):
...
def _build_split_bus_ids(original_bus_ids: np.ndarray) -> np.ndarray:
    ...
    bus_ids = original_bus_ids
    return np.concatenate((bus_ids, [_NEW_BUS_PLACEHOLDER_ID]))
...
def _validate_reassignment_targets(
    ...
    invalid_branch_indices = [
        int(branch_idx)
        for branch_idx in np.asarray(branches_connected_to_bus_b, dtype=np.int32)
    ...
    invalid_shunt_indices = [
        int(shunt_idx)
        for shunt_idx in np.asarray(shunt_connected_to_bus_b, dtype=np.int32)
        if not np.any(np.asarray(shunt_bus_indices[shunt_idx]) == bus_to_split)
    ]
    invalid_injection_indices = [
        int(injection_idx)
        for injection_idx in np.asarray(injections_connected_to_bus_b, dtype=np.int32)
...
def prepare_bsdf_test_context(
    ...
    dynamic_info_with_placeholders = replace_network_data(
    ...
    dynamic_info_split_manual = replace_network_data(
        dynamic_info_with_placeholders,
    ...
    branch_from_original = dynamic_info.branch_from_bus
    branch_to_original = dynamic_info.branch_to_bus
    v_mag_hat = dynamic_info_with_placeholders.bus_voltage_magnitudes
    theta_hat = dynamic_info_with_placeholders.bus_voltage_angles_rad
    ...
    mismatch_bsdf_reference = calculate_nodal_mismatch_network_data(
        dynamic_network_data=dynamic_info_split_manual,
        y_matrix=y_matrix_n1,
        jacobian_data=jacobian_data_with_extra_buses,
    )
    theta_base = dynamic_info_split_manual.bus_voltage_angles_rad
    vm_base = dynamic_info_split_manual.bus_voltage_magnitudes
    jacobian_data_split_manual = get_jacobian_data_from_network_data(
        dynamic_info_split_manual,
    )
    pvpq_indices = dynamic_info_split_manual.pvpq_buses_indices_pvpq_order
    pq_indices = dynamic_info_split_manual.pq_buses_indices
...
def run_reference_one_step(
    ...
    network_info_one_step = create_network_data_pypowsybl(net)
    dynamic_info_one_step = network_info_one_step.dynamic_network_data
    string_info_one_step = network_info_one_step.string_network_data
...
def run_reference_full_ac(
    ...
    network_info_full_ac = create_network_data_pypowsybl(net)
    dynamic_info_full_ac = network_info_full_ac.dynamic_network_data
    string_info_full_ac = network_info_full_ac.string_network_data
